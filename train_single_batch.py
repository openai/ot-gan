...
parser.add_argument('--layers_per_block', type=int, default=16)
parser.add_argument('--filters_per_layer', type=int, default=16)
...
distances = [tf.concat(dist_gen_gen,0) + tf.eye(args.nr_gpu * args.batch_size),
             tf.concat(dist_dat_dat,0) + tf.eye(args.nr_gpu * args.batch_size),
             tf.concat(dist_gen_dat,0)]

# use CPU/python to do assignment between samples
assignment_gen_gen = tf_match(distances[0], args.nr_matching_reps, args.matching_batch_size)
assignment_dat_dat = tf_match(distances[1], args.nr_matching_reps, args.matching_batch_size)
assignment_gen_dat = tf_match(distances[2], args.nr_matching_reps, args.matching_batch_size)
assignment_dat_gen = tf.gather(tf.range(args.nr_gpu * args.batch_size, dtype=tf.int32), assignment_gen_dat)

# get matched features
features_gen_gen_matched = tf.split(tf.gather(fg_all, assignment_gen_gen), args.nr_gpu, 0)
features_dat_dat_matched = tf.split(tf.gather(fd_all, assignment_dat_dat), args.nr_gpu, 0)
features_gen_dat_matched = tf.split(tf.gather(fd_all, assignment_gen_dat), args.nr_gpu, 0)
features_dat_gen_matched = tf.split(tf.gather(fg_all, assignment_dat_gen), args.nr_gpu, 0)

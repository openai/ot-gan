...
parser.add_argument('--data_dir', type=str, default='/home/hanzhang/Data/cifar10')
parser.add_argument('--save_dir', type=str, default='/home/hanzhang/Result/med_gan_cifar_last')
...
if args.model == 'dcgan':
    from models.dcgan_ori import generator, discriminator
...
with tf.Session() as sess:
    ...
    for epoch in range(current_epoch, 1000000):
        ...
        print("Iteration %d, time = %ds, #D updates %d, train distance before gen = %.6f, train distance before disc = %.6f, avg matching entropy = %.6f" % (
                epoch, time.time()-begin, dis_updates,
                mean_dist_gen[-1], mean_dist_disc[-1], np.mean(np_entropy)))
        sys.stdout.flush()
